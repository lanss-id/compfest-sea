...
class InfiniteGridMenu {
    ...
    private initTexture(): void {
        ...
        Promise.all(
            ...
            images.forEach((img, ndx) => {
                const cellX = ndx % this.atlasSize;
                const cellY = Math.floor(ndx / this.atlasSize);

                const cellAspect = cellSize / cellSize;
                ...
                const offsetX = cellX * cellSize + (cellSize - drawW) / 2;
                const offsetY = cellY * cellSize + (cellSize - drawH) / 2;
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
    ...
    return (
        <Modal>
            ...
            <div
                ref={detailsRef}
                className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-center text-center text-white transition-opacity duration-500"
                style={{ background: 'radial-gradient(circle, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0) 60%)' }}
            >
                {activeItem && (
                    <>
                        <h2 className="text-5xl font-bold text-white [text-shadow:0_4px_10px_rgba(0,0,0,0.8)]">
                            {activeItem.title}
                        </h2>
                        <p className="mt-4 max-w-sm text-lg text-neutral-200 [text-shadow:0_2px_6px_rgba(0,0,0,0.8)]">
